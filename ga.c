...
int N = 300; // size of  population
int nvar=13; // number of variables to be manipulated
...
int non_dom_sort(int pop,double y[][pop],int front[][pop+1]);
    ...
    */
	...
	int count = 0;
		...
		int domination = 0;
		    ...
		    //if (dom_by_combined[i][j] != 0) domination = 1;
                }
		/*if (domination == 0)
                {
                        int it = 0;
                        for (it = 0; it < nvar; it++) fprintf(ZerothFront, "%6.2f  ", var_pop[it][i]);
                        for (it = 0; it < nobj; it++) fprintf(ZerothFront, "%6.2f  ", obj_pop[it][i]);
                        fprintf(ZerothFront, "\n");
		}*/
                printf("\n");
            }
...
int non_dom_sort(int pop,double y[][pop],int front[][pop+1])
{
	int i,j,k,l,m,n,listN,var, obj_dom = 0;
	int dominate[N], nfront = 0;
	int partialP[pop+1], Ptemp[pop+1];
	int listofmember[pop][pop+1];

	// Initialization
	for (i = 0; i < pop; i++) dominate[i] = 0;

	for(i = 0; i < pop; i++) 
		for (j = 0; j < pop; j++) { listofmember[i][j] = 0; front[i][j] = 0;}

	for (i = 0; i < pop+1; i++) { Ptemp[i] = 0; partialP[i] = 0;}
	...
	for (i = 0; i < pop; i++)
	{
		for (j = i+1; j <pop; j++)
		{
			for (k = 0; k < nobj; k++)
			{
				if (y[k][i] <= y[k][j]) obj_dom++;
				else if (y[k][i] >= y[k][j]) obj_dom--;
	...
	while (Ptemp[0] > 0)
	{
		...
		Ptemp[0]=partialP[0];
		for(l = 1; l <= partialP[0]; l++) Ptemp[l] = partialP[l];
		partialP = 0;
		nfront += 1;
	}
