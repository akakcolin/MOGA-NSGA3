...
void remove_duplicates(FILE *output, double hyperplane[][nobj],
                       int size_hyperplane);
...
int read_var_input(char filename[], Bound bounds[]);
...
int select_pop(int rank[2 * N], int new_pop[N], int dom_by[2 * N][2 * N],
               int front[2 * N]); // fills population as close to N by rank
int normalize(double obj[nobj][2 * N],
              double norm_obj[nobj][2 * N]); // normalization of rank for niche
                                             // preservation
int associate(double norm_obj[nobj][2 * N],
              double hyperplane[nobj][size_hyperplane],
              int nearest_pointer[2 * N],
              double nearest_distance[2 * N]); // association of normalized
                                               // points to hyperplane
int niching(int pop_filled, int front[2 * N], int nearest_pointer[2 * N],
            int new_pop_point[N], double nearest_distance[2 * N]);

/* creates output files */
int write_pop_file(char fileName[], char allName[], double x[][N],
                   double y[][N], int type);

/* roulette selection for reproduction */
int select_crossover(int r[], double var_parent[nvar][N],
                     double var_child[nvar][N], Bound lower_upper[]);

/* mutant generation for remaining population */
int select_mutants(double var_parent[nvar][N], double var_child[nvar][N],
                   Bound lower_upper[]);

/* Matrix math functions */
double determinant(double[][nobj], double);
void cofactor(double[][nobj], double, double inv[nobj][nobj]);
void transpose(double[][nobj], double[][nobj], double,
               double inverse[nobj][nobj]);
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    double var_old[nvar][N];
    double var_new[nvar][N];
    double var_pop[nvar][2 * N];
    double obj_old[nobj][N];
    double obj_new[nobj][N];
    double obj_pop[nobj][2 * N];
    double norm_obj_pop[nobj][2 * N];
    int rank2[2 * N];
    int rank[N];
    double dom_by_combined[2 * N][2 * N];
    double dom_by_single[N][N];
    int front[2 * N];
    int hyper_pointer[2 * N];
    double hyper_distance[2 * N];
    int point_parent[N];
    double var_parent[nvar][N];
    double var_child[nvar][N];
    ...
    if (access(old_input, F_OK) != -1) { // returns TRUE if file exists
        ...
        for (i = 0; i < N; i++) {
            for (j = 0; j < nvar; j++)
                var_pop[j][i] = var_old[j][i];
            for (j = 0; j < nobj; j++)
                obj_pop[j][i] = obj_old[j][i];
        }
        for (i = 0; i < N; i++) {
            for (j = 0; j < nvar; j++)
                var_pop[j][N + i] = var_new[j][i];
            for (j = 0; j < nobj; j++)
                obj_pop[j][N + i] = obj_new[j][i];
        }
        ...
        non_dom_sort(2 * N, obj_pop, rank2,
                     dom_by_combined); // sorts population into ranks
        ...
        current_pop_size =
            select_pop(rank2, point_parent, dom_by_combined,
                       front); // populations with ranks until > N
        if (suppress_output == 0) {
            printf("\nSelected Population:\n");
        ...
        if (current_pop_size != N) {
            ...
            if (suppress_output == 0) {
                ...
                for (i = 0; i < 2 * N; i++) {
                    printf("%d %d %d %lf\n", i, front[i], hyper_pointer[i],
                           hyper_distance[i]);
                }
                printf("\n");
            }
            printf("Performing niching operation\n");
            niching(current_pop_size, front, hyper_pointer, point_parent,
                    hyper_distance);
            if (suppress_output == 0) {
        ...
        /* translates pointer to selected population into arrays for variable
         * and rank */
        if (suppress_output == 0)
        ...
        FILE *ZerothFront = fopen("ZerothFront.txt", "a");
        fprintf(ZerothFront, "%d\n", iterationNum);
        non_dom_sort(N, obj_new, rank,
                     dom_by_single); // sorts population into ranks
        ...
        for (i = 0; i < N; i++) {
            for (j = 0; j < nvar; j++) {
                var_parent[j][i] =
                    var_new[j]
                           [i]; // copies var_new to var_parent for consistency
                if (suppress_output == 0)
                    printf("%lf ", var_parent[j][i]);
            }
            if (suppress_output == 0)
...
int non_dom_sort(int pop, double y[][pop], int rank[pop],
                 int dom_by[pop][pop]) {
    int i, j, k;
    ...
    for (i = 0; i < pop;
         i++) { // loop through all members of the child + parent population
        int domination = 0;
        for (j = i + 1; j < pop;
             j++) { // loop through all other members for comparison purposes
            for (k = 0; k < nobj; k++) { // loop through all objectives
                if (y[k][i] < y[k][j]) {
                    obj_dom +=
                        1; // increments if objective is better for i than j
                } else if (y[k][i] > y[k][j]) {
                    obj_dom -=
                        1; // decrements if objective is worse for i than j
                }
            }
...
int select_pop(int rank[2 * N], int new_pop[N], int dom_by[2 * N][2 * N],
    ...
    for (i = 0; i < N; i++)
        new_pop[i] = -1;
    for (i = 0; i < 2 * N; i++)
        rank_temp[i] = rank[i]; // copies rank over to temp array
    ...
    while ((pop_size + current_rank_size) < N) {
        for (i = 0; i < 2 * N; i++) {
            if (front[i] == 1) {   // if member i is part of the front
                ...
                new_pop[pop_size] =
                    i;         // add "pointer" to member i to final population
                pop_size += 1; // increment size counter
                for (j = 0; j < 2 * N; j++) {
                    if (dom_by[i][j] == 1) {
                        rank_temp[j] -= 1; // reduces rank of member dominated
                                           // by removed pareto front member
                    }
                }
            }
        }
...
int normalize(double obj[nobj][2 * N], double norm_obj[nobj][2 * N]) {
    ...
    double s[2 * N];
    double smin = 0.0;
    double max[nobj][nobj];
    double obj_dummy[nobj][2 * N];
    double max_inv[nobj][nobj];
    double hyper_intercepts[nobj];
    double det;
    ...
    for (i = 0; i < nobj; i++) {
        for (j = 0; j < 2 * N; j++) {
            ...
            } else if (obj[i][j] < min[i])
                min[i] = obj_dummy[i][j]; // finds minimum value across combined
                                          // population
        }
        for (j = 0; j < 2 * N; j++) {
    ...
    for (i = 0; i < nobj; i++) {         // loop through objective axes
        for (j = 0; j < 2 * N; j++) {    // loop through population
            for (k = 0; k < nobj; k++) { // loop through objectives
                ...
                if ((obj_dummy[k][j] / epsilon_prime) > stemp)
                    stemp =
                        obj_dummy[k][j] /
                        epsilon_prime; // stores maximum value among objectives
            }

            s[j] = stemp; // one maximum non-axis objective per member of
                          // population
            stemp = 0.0;
        }

        for (j = 0; j < 2 * N;
             j++) { // finds index of the minimum of the set of maximums
            if (j == 0) {
                for (k = 0; k < i; k++)
                    if (j == index[k])
                        repeat = 1;
    ...
    for (i = 0; i < 2 * N; i++) { // loop over population
        ...
        for (j = 0; j < size_hyperplane; j++) { // loop over hyperplane
            ...
            for (k = 0; k < nobj; k++) { // loop over objectives
                ...
                sing_norm_obj[k] = norm_obj[k][i]; // extract current normalized objectives for
                                    // ease of reading
                vector_mult_acc += norm[k] * sing_norm_obj[k]; // accumulate for vector
                                                // muliplication of above
            }
            for (k = 0; k < nobj; k++) {
                dist_hold += (sing_norm_obj[k] - vector_mult_acc * norm[k]) *
                             (sing_norm_obj[k] - vector_mult_acc * norm[k]);
            }
            dist[i][j] = sqrt(dist_hold); // distance between hyperplane point
                                          // and population member
            if (j == 0) {
                nearest_distance[i] =
                    dist[i][j]; // initializes pointer to nearest hyperplane
                ...
                if (dist[i][j] < nearest_distance[i]) {
                    nearest_distance[i] =
                        dist[i]
                            [j]; // sets lowest distance and associated pointer
                    nearest_pointer[i] = j;
                }
            }
...
int niching(int pop_filled, int front[2 * N], int nearest_pointer[2 * N],
    ...
    for (i = 0; i < pop_filled; i++) {
        rho[nearest_pointer[new_pop_point[i]]] +=
            1; // fills niches according to current population
    }

    while (pop_filled < N) { // loops until parent population is... populated
        niche_min = rho[0];  // initializes minimum niche
        for (j = 1; j < size_hyperplane; j++) {
            if (rho[j] == niche_min)
                num_niche_min +=
                    1; // increments for multiple niches of smallest size
            if (rho[j] < niche_min) {
        ...
        unit_rand =
            (double)rand() / (double)(1.0 + RAND_MAX); // random number [0,1)
        nth_niche =
            (int)(unit_rand *
                  (double)
                      num_niche_min); // randomly selects which niche to fill

        for (j = 0; j < size_hyperplane; j++) {
            if (rho[j] == niche_min) { // if niche is a minimum niche
                if (nth_niche == count) {
                    niche_point =
                        j; // if niche is selected niche, set pointer and break
                    break;
                }
                if (nth_niche > count)
                    count += 1; // else, increment niche counter
            }
        }

        if (niche_min == 0) {             // if minimum niche is empty
            for (j = 0; j < 2 * N; j++) { // loop over population
                if (front[j] == 1) {      // if point is member of the front
                    /* if the niche of the member of the front matches the
                     * selected niche */
                    if (nearest_pointer[j] == niche_point) {
                        if (initialized ==
                            0) { // if this is first member of niche
                            min_dist = nearest_distance
                                [front[j]];  // sets first distance to minimum
                            new_point = j;   // sets first pointer to shortest
                                             // distance member of niche
                            initialized = 1; // sets boolean to true
                        } else if (nearest_distance[j] < min_dist) {
                            min_dist = nearest_distance[j]; // sets new distance
                                                            // to minimum
                            new_point = j; // sets new pointer to shortest
                                           // distance member of niche
                        }
                    }
                }
            }
            if (new_point < 0) { // if no point in niche exists within front
                rho[niche_point] =
                    2 * N + 1; // niche  removed from consideration
            } else {
                new_pop_point[pop_filled] =
                    new_point;         // point added to parent population
        ...
        } else {       // if miniumum niche is not empty
            count = 0; // reset counter
            for (j = 0; j < 2 * N; j++) {
                if (front[j] == 1) { // if point is member of front
                    /* if the niche of the member of the front matches the
                     * selected niche */
                    if (nearest_pointer[j] == niche_point)
                        count += 1; // increment counter of number of members in
                                    // niche
                }
            }
            if (count == 0) { // no point in niche exists within front
                rho[niche_point] =
                    2 * N + 1; // removes niche from consideration
            } else {
                unit_rand = (double)rand() /
                            (double)(1.0 + RAND_MAX); // random number [0,1)
                new_point =
                    (int)(unit_rand * (double)count); // randomly picks member
                ...
                for (j = 0; j < 2 * N; j++) {
                    if (front[j] == 1) { // if point is member of front
                        ...
                        if (nearest_pointer[j] == niche_point) {
                            if (count == new_point) {
                                new_pop_point[pop_filled] =
                                    j; // adds to population
                                pop_filled += 1;
                                rho[niche_point] += 1;
                                front[j] = 0;
                                break;
                            }
                            count += 1; // increment counter of number of
                                        // members in niche
                        }
                    }
                }
...
int select_crossover(int r[], double var_parent[nvar][N],
    ...
    for (i = 0; i < N; i++) {
        if (rmax < (r[i] + 1))
            rmax = r[i] + 1; // finds latest rank included in population
    }
    for (i = 0; i < N; i++) {
        rsum +=
            (rmax - r[i] - 1); // sums up ranks (+1 accounts for first rank = 0)
        for (j = 0; j < nvar; j++)
            var_child[j][i] = 0.0; // initializes var_child
    }

    for (i = 0; i < N; i++)
        rprob[i] = (rmax - r[i] - 1) /
                   rsum; // lower ranks have higher probability of acceptance
    raprob[0] = rprob[0];
    for (i = 1; i < N; i++)
        raprob[i] = raprob[i - 1] + rprob[i];
    ...
    for (i = 0; i < crossover_n; i++) {
        ...
        for (j = 0; j < nvar; j++) {
            do {
                ...
                // var_child[j][i] =
                // rand_num*var_parent[j][mating_pair[0]]+(1-rand_num)*var_parent[j][mating_pair[1]];
                double ui = (double)rand() / (double)RAND_MAX;
                double yi = (1 + 2 * alpha) * ui - alpha;
                var_child[j][i] = yi * var_parent[j][mating_pair[0]] +
                                  (1 - yi) * var_parent[j][mating_pair[1]];
            } while (var_child[j][i] < lower_upper[j].x ||
                     var_child[j][i] > lower_upper[j].y);
        }
    }
    return (0);
}
...
int select_mutants(double var_parent[nvar][N], double var_child[nvar][N],
    ...
    int i, j;
    // calculates portion of population that are mutants
    int n_mutants = (int)ceil(mutation_P * N);
    // number of variables to mutate per mutation
    int n_var_mutate = (int)ceil(mutation_rate * nvar);
    ...
    double BM_uniform_1;
    double BM_uniform_2;
    double BM_normal; // rands for box-muller method - normal distributed rand
    ...
    for (i = 0; i < n_mutants; i++) {
        ...
        parent_mutate =
            (int)(unit_rand1 * (double)(N)); // picks parent to mutate
        printf("Population to mutate %d \n", parent_mutate);
        for (j = 0; j < nvar; j++)
            var_child[j][n_start + i] =
                var_parent[j][parent_mutate]; // copies variables of parent
        printf("Total variables in a population to be mutated %d \n",
               n_var_mutate);

        for (j = 0; j < nvar; j++) {

            var_mutate = j; //(int)(unit_rand2*(double)(nvar)); // picks
            ...
            printf("Lower and upper bound of variable %d is %lf and %lf\n",
                   var_mutate, lower_upper[var_mutate].x,
                   lower_upper[var_mutate].y);
            do {
                sigma = 0.5 *
                        (lower_upper[var_mutate].y - lower_upper[var_mutate].x);
                BM_uniform_1 = (double)rand() / (double)RAND_MAX;
                BM_uniform_2 = (double)rand() / (double)RAND_MAX;
                BM_normal =
                    sqrt(-2 * log(BM_uniform_1)) * cos(2 * M_PI * BM_uniform_2);
                mutant = var_child[var_mutate][n_start + i] + sigma * BM_normal;
            } while (mutant < lower_upper[var_mutate].x ||
                     mutant > lower_upper[var_mutate].y);
            printf("Lower and Upper bound of selected variable %d is %lf and "
                   "%lf \n",
                   var_mutate, lower_upper[var_mutate].x,
                   lower_upper[var_mutate].y);
            var_child[var_mutate][n_start + i] += sigma * BM_normal;
            printf("Value of the selected population %d and corresponding "
                   "variable %d after mutation is %lf\n",
                   n_start + i, var_mutate, var_child[var_mutate][n_start + i]);
        }
    }
    return (0);
...
int write_pop_file(char fileName[], char allName[], double x[][N],
    ...
    output = fopen(
        fileName,
        "w"); // creates output file, overriding previous file if applicable
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < nvar; j++) {
            ...
            if (type == 1)
                fprintf(all_out, "%f ", x[j][i]);
        }
        if (type == 1) {
            for (j = 0; j < nobj; j++) {
                fprintf(output, "%f ",
                        y[j][i]); // prints objectives in a line for parent only
...
void create_hyperplane() {
    ...
    int part_divis[nDivision][nobj]; // if num of divisions is larger than num
                                     // of objectives
    int part_obj[nobj]
                [nobj]; // if num of objectivies is larger than num of divisions
    FILE *output, *create;
    ...
    if (nDivision > nobj) { // if the number of divisions is larger than the
                            // number of objectives
        for (i = 0; i < nobj; i++) {
            for (j = 0; j < nDivision; j++)
                part_divis[j][i] = 0; // initialization
        }
        n_part = partition(part_divis, n_part);
    } else {
        for (i = 0; i < nobj; i++) {
            for (j = 0; j < nobj; j++)
                part_obj[j][i] = 0; // initialization
        }
        n_part = partition(part_obj, n_part);
    }

    size_hyperplane =
        obj_factorial * (n_part + 1); // size of hyperplane is number of unique
                                      // partitions * permutaions per partition
    for (i = 0; i < n_part; i++) {
        if (nDivision > nobj) {
            for (j = 0; j < nobj; j++)
                sing_part[j] =
                    part_divis[i][j]; // nDivision; // extract single partition
        } else {
            for (j = 0; j < nobj; j++)
                sing_part[j] =
                    part_obj[i][j]; // nDivision; // extract single partition
        }
    ...
    for (i = 0; i < size_hyperplane; i++) {
        for (j = 0; j < nobj; j++)
            hyperplane[i][j] = 0.0; // initialization
    }
    remove_duplicates(
        output, hyperplane,
        size_hyperplane); // removes duplicates from hyperplane document
    fclose(output);
}
...
int partition(int f_p[][nobj], int index) {
    ...
    while (1) {
        if (nDivision > nobj) { // save current partition
            if (p[nobj] != 0)
                break; // partition has size > nobj
        }
        for (i = 0; i < nDivision; i++)
            f_p[index][i] = p[i]; // copy single partition to set
        ...
        if (k < 0)
            return (index); // if k < 0, all the values are 1 so there are no
                            // more partitions
        ...
        p[k + 1] =
            rem_val; // Copy rem_val to next position and increment position
        k++;
    }
...
void permute(FILE *output, double sing_part[], int start) {
    ...
    if (start == nobj) { // if a full permutation is filled
        for (i = 0; i < nobj; i++) {
            fprintf(output, "%.5lf ",
                    sing_part[i] /
                        nDivision); // print permutation to hyperplane doc
        }
        fprintf(output, "\n");
        return;
    }
    for (i = start; i < nobj; i++) {
        ...
        permute(output, sing_part,
                start + 1); /* fixes one first digit and calls permutation on
                               the rest */
...
void remove_duplicates(FILE *output, double hyperplane[][nobj],
    ...
    SKIP:
        if (feof(output))
            break; // checks for EoF, breaks if found. goto position
        if (current_line > 0) {      // for all but first loop
            fgets(point, 9, output); // throws out "\n" between partitions
        }
        for (i = 0; i < nobj;
             i++) { // extracts a partition and saves to temporary storage
        ...
        if (current_hyper > 0) { // all but first loop
            for (i = 0; i < current_hyper;
                 i++) { // loop through current extracted partitions
                for (j = 0; j < nobj; j++) {
                    if (temp[j] == hyperplane[i][j])
                        match++; // check is partition is duplicate
                }
                if (match == nobj) { // if partition is a duplicate
                    match = 0;       // reset match counter
                    duplicates[k] =
                        current_line; // flag current position in doc
                    current_line++;
                    k++;       // increment duplicate counter
                    goto SKIP; // use goto to skip step in which partition is
                               // added to hyperplane
                }
                match = 0; // reset match counter
            }
    ...
    true_hyper =
        fopen("hyperplane.in", "w"); // open doc for hyperplane w/o duplicates
...
void transpose(double num[nobj][nobj], double fac[nobj][nobj], double r,
               double inverse[nobj][nobj]) {
    int i, j;
    double b[nobj][nobj];
    double d;
