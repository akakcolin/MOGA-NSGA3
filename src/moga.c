...
void modify_input(double A[3], double rho[3], double B[3], double lambda[3],
                  double gamma_3b[2], double theta);
...
double CubicSplineInterp(int N, double *x, double *f, Range *limit, int ref_N,
                         double *ref_x, double *ref_f, int id);
// read in band data for dispersion curve, contains hardcoded values for band
// data structures
void ReadData(int numBands, double x_vasp_data[][20000],
              double f_vasp_data[][20000], double x_gulp_data[][20000],
              double f_gulp_data[][20000], int state);
...
int main(int argc, char *argv[]) {
    ...
    for (j = 0; j < iteration_num;
         j++) { // recursively optimizes for number of iterations specified
        ...
        for (i = myid; i < population_num;
             i += nprocs) { // loops through population list, assigning members
                            // to processors

            sprintf(folder, "%d",
                    i); // creates a folder to hold all analysis on current line
            ...
            // end
            printf("%s\n", variables);
            processor_run(myid, j, folder,
                          variables); // runs analysis on line input
        }
        fclose(input);
...
void file_copy(char file[], char copyTo[], int bands,
               int state) { // creates a copy of a file
...
void modify_input(double A[3], double rho[3], double B[3], double lambda[2],
                  double gamma_3b[2], double theta) {
    ...
    for (i = 0; i < 3;
         i++) { // modifies 2-body terms 3-pairs in total Se-Se, Mo-Se, Mo-Mo

        fseek(ff, 11, SEEK_CUR); // goes to first modified value
        sprintf(str, "%6.3lf   %6.3lf   %7.4lf", A[i], rho[i],
                B[i]); // Modifies A, rho and B to every pair corresponding to
                       // the tuple in ga.in file

        fputs(str, ff); // updates first line w/ correct values, current cursor
                        // position at end of B
    ...
    for (i = 0; i < 2; i++) {
        ...
        sprintf(str, "%7.4lf  %7.4lf %6.3lf %6.3lf", lambda[i], theta,
                gamma_3b[i], gamma_3b[i]);

        fputs(str, ff); // updates first line w/ correct values, current cursor
                        // position at end of rmax
...
void read_output(double *err_a, double *elast) {
    ...
    char lines[200],
        search_string[] = "  Comparison of initial and final structures : ", ch;
    while (fgets(lines, 200, af) !=
           NULL) { // search output file for ^ test string
        int i;
    ...
    sscanf(lines, "%s %lf %lf %lf %s %lf", tempStr1, &temp1, &temp2, &temp3,
           tempStr2, &err_dum); // assigns value of percent error in a
    fgets(lines, 200, af);      // reads line in
    sscanf(lines, "%s %lf %lf %lf %s %lf", tempStr1, &temp1, &temp2, &temp3,
           tempStr2, &err_dum1);
    *err_a = sqrt(pow(err_dum, 2) + pow(err_dum1, 2));
    ...
    /*line_down = 0;
    while (line_down !=1) { // moves down to elastic constant "c" (for elastic
    fit) if((ch = fgetc(af)) == '\n') line_down++;
    }*/
    ...
    // printf("Extracting C\n");
    sscanf(lines, "%s %lf %lf %lf %s %lf", tempStr1, &temp1, &c, &temp2,
           tempStr2, &temp3); // assigns value of elastic constant "c" (c_final)
    ...
    *elast = fabs(elast_calc - elast_real) / elast_real *
             100; // percent error in elastic constant
...
void ReadData(int numBands, double x_vasp_data[][20000],
              double f_vasp_data[][20000], double x_gulp_data[][20000],
              double f_gulp_data[][20000], int state) {
    int regionsPerBand = 3;
    int pointsPerRegion_vasp = 101;
    ...
    int points_GM_gulp = regions[state][0];
    int points_MK_gulp = regions[state][1];
    int points_KG_gulp = regions[state][2];

    double temp_x, temp_y;

    double phonon_shift[4] = {
        0, regions[state][0], regions[state][0] + regions[state][1],
        regions[state][0] + regions[state][1] + regions[state][2]};
    ...
    const char *phonon_file[3] = {"G_M/phonon.disp", "M_K/phonon.disp",
                                  "K_G/phonon.disp"};

    char lines_gulp[200];
    char lines_band[200];
    char lines_band_ph[200];

    FILE *vasp, *gulp[4];
    if (state == 0)
        vasp = fopen("../../UTIL/band0.dat", "r");
    else if (state == 1)
        vasp = fopen("../../UTIL/band1.dat", "r");
    else if (state == 2)
        vasp = fopen("../../UTIL/band2.dat", "r");

    /* Reading Vasp File band.dat */
    // vasp = fopen(band_file, "r");
    if (vasp == NULL) {
        printf(
            "Unable to open file for reading dispersion data .... exiting \n");
        exit(1);
    }
    while (lineNum_vasp < band_offset) {
    ...
    for (i = 0; i < numBands; i++) {
        ...
        for (j = 0; j < regionsPerBand; j++) {
            ...
            while (strcmp(lines_band, "\n") != 0) {
                ...
                // x_vasp_data[i][k], f_vasp_data[i][k]);
                k++;
                lineNum_vasp++;
            }
    ...
    for (i = 0; i < regionsPerBand; i++) {
        ...
        if (gulp[i] == NULL) {
            printf("Unable to open file %s for reading dispersion data .... "
                   "exiting \n",
                   phonon_file[i]);
            exit(1);
        }
        while (lineNum_gulp < gulp_offset) {
            fgets(lines_gulp, 200, gulp[i]);
            lineNum_gulp++;
        }
        if (i == 0)
            curPoints = points_GM_gulp;
        else if (i == 1)
            curPoints = points_MK_gulp;
        else if (i == 2)
            curPoints = points_KG_gulp;
...
double CubicSplineInterp(int N, double *x, double *f, Range *limit, int ref_N,
    ...
    if (fw == NULL) {
        printf("Cannot open file to write.... exiting \n");
        exit(1);
    }
    ...
    for (i = 0; i <= N - 1; i++) {
        if (i < N - 1) {
            ...
            d[i] = x[i + 1] - x[i];
            r[i] =
                6 * (f[i + 2] - f[i + 1]) / h[i] - 6 * (f[i + 1] - f[i]) / d[i];
        } else if (i == N - 1) {
            d[i] = x[i + 1] - x[i];
        }
    }
    ...
    for (j = 0; j < ref_N; j++) {
        x1 = ref_x[j];
        if (limit->min_x > x1 || limit->max_x < x1)
            continue;
...
double ErrorPhononDispersion(int state) {
    ...
    double ref_x[regionsPerBand * pointsPerRegion_vasp],
        ref_f[regionsPerBand * pointsPerRegion_vasp];
    ...
    for (i = 0; i < numBands; i++) {
        if (i < 3) {
            ...
            w[i] = 0.1;
            normalizingFactor += 0.1;
        }
    }

    double SquaredError = 0.0;

    double min_x = 10000000000.0, max_x = -10000000000.0, min_f = 10000000000.0,
           max_f = -10000000000.0;

    Range *lim = (Range *)malloc(sizeof(Range));
    ...
    for (BandNum = 0; BandNum < numBands; BandNum++) {
        ...
        Error[BandNum] = CubicSplineInterp(
            530, x, f, lim, regionsPerBand * pointsPerRegion_vasp, ref_x, ref_f,
            BandNum);

        SquaredError += w[BandNum] * Error[BandNum];
    }
    // for (BandNum=0; BandNum < numBands; BandNum++) printf("%10.6f \t ",
    // Error[BandNum];
    free(lim);
    return SquaredError;
    // return
    // (SquaredError/(normalizingFactor*regionsPerBand*pointsPerRegion_vasp));
}
...
void initialize_population(int population_num) {
    ...
    // double variables[14] = {2.0, 6.0, 3.0, 1.6, 0.4,
    // 0.4, 29.0, 12.0, 33.0, 66.0, 23.0, 1.70, 5.70, 80.0}; double variables[14]
    // =
    // {7.311202,3.182849,1.751833,0.263154,0.276447,34.792870,7.942051,27.888579,87.012161,17.268118,2.098628,5.993827,77.305379}
    double variables[14] = {2.3,       5.710990,  2.890970,  1.6533,
                            0.524863,  0.308964,  37.596895, 13.340301,
                            33.898837, 63.438181, 18.979162, 2.007693,
                            5.657942,  78.174734};
    ...
    for (i = 0; i < population_num; i++) {
        for (j = 0; j < 14; j++) {
            if (j <= 2)
                rand_frac = rand() / (double)(RAND_MAX) * (2 * 0.03) - 0.03;
            else if (j > 2 && j <= 12)
                rand_frac = rand() / (double)(RAND_MAX) * (2 * frac_perturb) -
                            frac_perturb;
            else if (j == 13)
                rand_frac = rand() / (double)(RAND_MAX) * (2 * 0.05) - 0.05;
...
void processor_run(int id, int iter_num, char folder[], char inputs[]) {
    ...
    // initialize variables (make global only if using private copies in this
    // subroutine (OpenMPI))
    double A[3], rho[3], B[3], lambda[2], gamma_3b[2],
        theta; // variables modified in the force field file
    ...
    sscanf(inputs, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
           &A[0], &A[1], &A[2], &rho[0], &rho[1], &rho[2], &B[0], &B[1], &B[2],
           &lambda[0], &lambda[1], &gamma_3b[0], &gamma_3b[1], &theta);
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            file_copy(file, CopyTo, 0,
                      i); // 0 as last argument is just a place holder for
                          // creating a cell file at given path. It will
                          // overwritten for different regions
        }

        chdir(path); // enter folder

        modify_input(A, rho, B, lambda, gamma_3b,
                     theta); // modifies "forcefield" file
        ...
        if (i == 0)
            fprintf(gulp_input, "phon nofreq\n");
        else if (i == 1)
        ...
        for (k = 0; k < 3; k++) {
            ...
            system("/home/jian/bin/gulp < in.gulp > afterfit.out"); // runs gulp

            chdir(".."); // exits new directory
        }

        chdir(segment[0]); // arbitrary choice of segment for objectives 1 & 2
                           // (same for all)
        // printf("%d Extracting elastic constant \n", id);
        if (i == 1) {
            read_output(&err_a,
                        &elast); // extracts objective 1 & 2: error in lattice
                                 // constant a and error in elastic constant
            err_a *= weight_a;
            elast *= weight_c11;
        }
        ...
        chdir(".."); // moves back to 'folder' directory to read all segments
                     // for phonon dispersion chi squared calculation
    ...
    fprintf(output,
            ...
            "%lf %lf %lf\n",
            A[0], A[1], A[2], rho[0], rho[1], rho[2], B[0], B[1], B[2],
            lambda[0], lambda[1], gamma_3b[0], gamma_3b[1], theta, err_a, elast,
            chi_sq[0], chi_sq[1], chi_sq[2]);
